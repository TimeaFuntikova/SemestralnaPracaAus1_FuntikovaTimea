...
#include "../SemestralnaPracaAus1_FuntikovaTimea/table/data_item.h"
...
namespace structures
{
    ...
    inline Structure& LinkedList<T>::assign(Structure& other)
    {
        if (this != &other)
        {
            ...
            while (otherItem != nullptr) //kym nepride na koniec
            {
    ...
    inline bool LinkedList<T>::equals(Structure& other)
    {
        ...
        if (size_ == otherList.size())
        {
            ...
            {
                if (!(this->getItemAtIndex(i)->accessData() == otherList.getItemAtIndex(i)->accessData()))
                {
                    return false;
                }
            }
    ...
    inline void LinkedList<T>::insert(const T& data, int index)
    {
        ...
        else
        {
            ...
            LinkedListItem<T>* novyItem = new LinkedListItem<T>(data);
            if (index == 0) // zaciatok
            {
                novyItem->setNext(first_);
                first_ = novyItem;
            }
            else {
                LinkedListItem<T>* predchodza = this->getItemAtIndex(index - 1);
                LinkedListItem<T>* nasledovnik = predchodza->getNext();
                predchodza->setNext(novyItem);
                novyItem->setNext(nasledovnik);
            }
            size_++;
        }
    }
    ...
    {
        int indexPrvku = getIndexOf(data);
        if (indexPrvku != -1)
        {
            removeAt(indexPrvku);
            return true;
    ...
    inline T LinkedList<T>::removeAt(int index)
    {
        ...
        else
        {
            ...
            {
                LinkedListItem<T>* predchodca = this->getItemAtIndex(index - 1);
                deleteItem = predchodca->getNext();
                predchodca->setNext(deleteItem->getNext());
                if (deleteItem == last_)
                {
                    last_ = predchodca;
                }
    ...
    {
        LinkedListItem<T>* nextPtr;
        while (first_ != nullptr)
        {
            nextPtr = first_->getNext();
            delete first_;
            first_ = nextPtr;
        }
        size_ = 0;
    ...
    {
        //nemam index ale pos je pointer -> accessData()
