...
#include "../SemestralnaPracaAus1_FuntikovaTimea/List.h"
...
namespace structures
{
	...
	inline bool ArrayList<T>::equals(Structure& other)
	{
		...
		return array_->equals(*(otherArrayList->array_));
	...
	{
		//Utils::rangeCheckExcept(index, size_, "Invalid Index"); -- aka je zlozitost tohto?
	...
	inline void ArrayList<T>::add(const T& data)
	{
		...
		array_->at(size_) = data; // pridavanie na koniec zoznamu
		size_++;
	}
	...
	inline void ArrayList<T>::insert(const T& data, int index)
	{
		...
		{
			Utils::rangeCheckExcept(index, size_, "range exception");
	...
	{
		position_ = dynamic_cast<ArrayListIterator&>(other).position_;
		return *this;
	}
	...
	template<typename T>
	inline T ArrayList<T>::ArrayListIterator::operator*() //getIndexOf?
	{
